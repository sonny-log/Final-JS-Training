...
function display_body(data, now) { 
  ...
  create_indicator(indicator, data, now, items);
  update_card_content(data.list, items);
  add_touch_event(content, indicator, data, items);
  add_drag_event(items);
}
...
function create_card(i, size, now, max) { 
  ...
  for (let j = 0; j < max; j++) {
    ...
    `
      ...
      <div draggable="true" class="image" style="background-image: url('img/plus.png')"></div>
...
function add_drag_event(items) {
  let dragging_item;
    items.forEach(item =>{
       /* if(item.style.backgroundImage === 'url("img/plus.png")') {
        item.setAttribute('draggable', false); // 드래그 X 속성 넣기
        item.addEventListener('dragover', (e) =>{
          e.preventDefault();
          let data = e.dataTransfer.getData('Text');
          
          data = item.style.backgroundImage;
          item.style.backgroundImage = data;
        })
        item.addEventListener('drop', (e) =>{
          e.preventDefault();
          console.log('Before drop item : ' + item.style.backgroundImage);
          if (item.style.backgroundImage === 'url("img/plus.png")') {
          ...
          item.style.backgroundImage = data;
          console.log(data);
          console.log('After drop item : ' + item.style.backgroundImage);
        })
      }  */
      item.addEventListener('dragstart', (e) =>{
        console.log(item);
        console.log('start item :' + item.style.backgroundImage);
        e.dataTransfer.setData('Text', item.style.backgroundImage);
      ...
      /*item.addEventListener('dragenter', (e) =>{
        e.preventDefault();
        console.log('enter ele :' + item.style.backgroundImage);
      }) */
      item.addEventListener('drop', (e) =>{
        e.preventDefault();
        /* dragging_item.style.backgroundImage = 'url("img/plus.png")';
        dragging_item.style.backgroundSize = '50% 50%'; */
        dragging_item.style.backgroundImage = item.style.backgroundImage;
        console.log('Before drop item : ' + item.style.backgroundImage);
        if (item.style.backgroundImage === 'url("img/plus.png")') {
          item.style.backgroundSize = '100% 100%';
          dragging_item.style.backgroundSize = '50% 50%'
