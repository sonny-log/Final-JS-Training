  ...
  cards.forEach(card => {
    ...
    card.children[0].addEventListener('drop', (e) => {
      ...
      if (e.target.style.backgroundImage == 'unset') {
        ...
        dragging_card.children[0].style.backgroundImage = 'unset';
        dragging_card.children[0].setAttribute('draggable', false);

        list[start_index].card = Number(drop_ID.substring(7));
      } else {
        dragging_card.children[0].style.backgroundImage = e.target.style.backgroundImage;
        e.target.style.backgroundImage = data;

        list[drop_index].card = list[start_index].card;
        list[start_index].card = Number(drop_ID.substring(7));
      }
