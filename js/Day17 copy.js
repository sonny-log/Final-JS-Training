...
function create_card(i, size, now, max) {
  ...
  for (let j = 0; j < max; j++) {
    ...
    html +=
      ...
      <div class="image"></div>
      <div class="text" draggable="false"> ` + (item_wrap + 1) + `_` + item_card + ` </div>
      </div>
    `;
  }
  return html;
...
function add_drag_event2() {
  ...
  items.forEach(item => {
    ...
    item.addEventListener('dragover', (e) => {
      console.log('dragover item :' + item.style.backgroundImage);
      e.preventDefault();
    })
    item.addEventListener('drop', (e) => {
      console.log('drop item :' + e.target.style.backgroundImage);
      console.log('drop item :' + item.style.backgroundImage);
    ...
    item.addEventListener('dragend', (e) => {
      console.log('dragend item :' + item.style.backgroundImage);
